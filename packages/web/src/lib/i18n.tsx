...
const zh = {
  ...
  'sidebar.archive': '归档',
  'sidebar.executors': '执行器',
  'sidebar.executorsHint': '管这台机器上有哪几位干活的：名字 + CLI + 模型',
  ...
  // ── 执行器 ─────────────────────────────────────────────
  'executors.title': '执行器',
  'executors.hint': '一个起了名字的「哪个 CLI + 哪个模型」。建一次，处处用它的名字。',
  'executors.new': '新建执行器',
  'executors.name': '名字',
  'executors.namePlaceholder': '大壮',
  'executors.nameHint': '在对话里 @ 它用的就是这个名字，所以不能有空格。',
  'executors.provider': 'CLI',
  'executors.model': '模型',
  'executors.modelDefault': '用这个 CLI 自己的默认',
  'executors.create': '建好',
  'executors.cancel': '取消',
  'executors.save': '保存',
  'executors.edit': '改',
  'executors.delete': '删掉',
  'executors.deleteHint': '删掉之后，指着它的卡回到"用默认执行器"，卡本身一张不动。',
  'executors.default': '默认',
  'executors.setDefault': '设为默认',
  'executors.defaultHint': '不指定执行器的卡、以及右边那块聊天，用的都是它。',
  'executors.empty': '还没有执行器。建一个 —— 聊天和派活都要有人干。',
  'executors.loadFailed': '这份名单没读出来，稍后再试',
  'executors.saveFailed': '没保存成功，稍后再试',
  'executors.deleteFailed': '没删掉，稍后再试',
  'executors.defaultFailed': '默认执行器没换成，稍后再试',
  'executors.noProviders': '本机没有探测到任何 Agent CLI，先装一个再来。',
  'executors.usedBy': '{n} 张卡',
  'executors.tasksHint': '眼下有几张卡指名交给它',

  // ── 列 ─────────────────────────────────────────────────
  'column.backlog.hint': '想法池 · Agent 不可领',
  'column.ready.hint': '队列 · 等待认领',
  'column.running.hint': 'Agent 执行中',
  'column.loop.hint': '排着的在上、在跑的在下',
  'column.loop.running': '{n} 张正在跑',
  ...
  'talk.nextRoundBy': '这一轮交给 {name} —— 想换人就 @ 他',
  'talk.mentionHint': '@{name}，把这一轮交给他',
  ...
  // ── 右侧面板：建卡 + 一张卡的对话与配置 ─────────────────
  'chat.title': '聊天',
  'chat.hint': '跟 {name} 聊清楚要做什么，谈拢了它来出卡',
  'chat.hintNobody': '还没有执行器 —— 建一个才聊得起来',
  'chat.createHint': '说说你想做什么。它会问清楚，然后给出一份任务草案让你过目。',
  'chat.createPlaceholder': '要做点什么？说说看 —— @ 换个人聊，# 引用一张卡',
  'chat.create': '建卡',
  'chat.send': '发送',
  'chat.thinking': '{name} 正在想…',
  'chat.sendFailed': '这句话没发出去，稍后再试',
  'chat.turnFailed': '{name} 那一轮没跑成：{detail}',
  'chat.noExecutor': '没有可用的执行器 —— 去左侧「执行器」建一个。',
  'chat.clear': '清空这段对话',
  'chat.clearFailed': '没清掉，稍后再试',
  'chat.proposal': '任务草案',
  'chat.proposalHint': '过目一下。建出来的卡就是这些字。',
  'chat.acceptance': '验收标准',
  'chat.adoptBacklog': '放进 Backlog',
  'chat.adoptBacklogHint': '先记下来，回头再排',
  'chat.adoptReady': '直接进 Loop',
  'chat.adoptReadyHint': '排进队列 —— 自动认领开着的话，很快就有人捡走',
  'chat.adopted': '已经建成 {id}',
  'chat.adoptFailed': '这张卡没建成，稍后再试',
  'chat.you': '你',
  ...
  'editor.executor': '交给谁',
  'editor.executorHint': '这张卡固定归他；在对话里 @ 别人就是换人',
  'editor.executorDefault': '没指定 —— 眼下是默认执行器「{name}」在干',
  'editor.executorNone': '还没有执行器。去左侧「执行器」建一个。',
  'editor.executorAny': '默认执行器',
  'editor.executorFallback': '默认执行器（{name}）',
  ...
  'err.executor-duplicate': '已经有同名的执行器了。名字得能一眼认出是谁。',
  'err.executor-empty': '执行器要有名字。',
  'err.executor-too-long': '名字太长了。',
  'err.executor-illegal-chars': '名字里不能有空格、@ 或 #。@ 到哪儿算完，全靠这条规矩。',
  'err.executor-unknown-provider': '本机没有探测到这个 CLI。装好它，或者换一个。',
  'err.executor-not-found': '这个执行器已经不在了，可能刚被删掉。',
  'err.chat-unavailable': '当前实例没有装对话服务，聊不了天。',
  'err.proposal-not-found': '这份草案已经不在了。',
  'err.already-adopted': '这份草案已经建成卡片了。',
...
const en: Record<MessageKey, string> = {
  ...
  'sidebar.executors': 'Executors',
  'sidebar.executorsHint': 'Who does the work on this machine: a name + a CLI + a model',
  ...
  'executors.hint': 'A named pairing of a CLI and a model. Create it once, then use the name everywhere.',
  'executors.new': 'New executor',
  'executors.name': 'Name',
  'executors.namePlaceholder': 'Ace',
  'executors.nameHint': 'This is the name you @ in conversations, so it can’t contain spaces.',
  'executors.provider': 'CLI',
  'executors.model': 'Model',
  'executors.modelDefault': 'Whatever this CLI defaults to',
  'executors.create': 'Create',
  'executors.cancel': 'Cancel',
  'executors.save': 'Save',
  'executors.edit': 'Edit',
  'executors.delete': 'Delete',
  'executors.deleteHint': 'Cards pointing at it fall back to the default executor. No card is touched.',
  'executors.default': 'Default',
  'executors.setDefault': 'Make default',
  'executors.defaultHint': 'Cards with no executor — and the chat panel — all use this one.',
  'executors.empty': 'No executors yet. Create one — chatting and dispatching both need someone to do the work.',
  'executors.loadFailed': 'Could not load the list — try again',
  'executors.saveFailed': 'Could not save that — try again',
  'executors.deleteFailed': 'Could not delete it — try again',
  'executors.defaultFailed': 'Could not change the default — try again',
  'executors.noProviders': 'No agent CLI detected on this machine. Install one first.',
  'executors.usedBy': '{n} cards',
  'executors.tasksHint': 'How many cards are assigned to them right now',
  ...
  'column.loop.hint': 'Queued on top, running below',
  'column.loop.running': '{n} running',
  ...
  'card.executor': 'Assigned to {name}',
  ...
  'talk.nextRoundBy': 'This round goes to {name} — @ someone else to switch',
  'talk.mentionHint': '@{name} — hand this round to them',
  ...
  'chat.title': 'Chat',
  'chat.hint': 'Talk it through with {name}; once it’s clear, they draft the card',
  'chat.hintNobody': 'No executors yet — create one to start talking',
  'chat.createHint': 'Say what you want done. They’ll ask until it’s clear, then hand you a draft card to look over.',
  'chat.createPlaceholder': 'What needs doing? — @ to switch who’s talking, # to reference a card',
  'chat.create': 'Create',
  'chat.send': 'Send',
  'chat.thinking': '{name} is thinking…',
  'chat.sendFailed': 'Could not send that — try again',
  'chat.turnFailed': '{name} couldn’t finish that turn: {detail}',
  'chat.noExecutor': 'No executor available — create one under Executors on the left.',
  'chat.clear': 'Clear this conversation',
  'chat.clearFailed': 'Could not clear it — try again',
  'chat.proposal': 'Draft card',
  'chat.proposalHint': 'Look it over. The card gets created exactly as written.',
  'chat.acceptance': 'Acceptance criteria',
  'chat.adoptBacklog': 'Into Backlog',
  'chat.adoptBacklogHint': 'Park it — schedule it later',
  'chat.adoptReady': 'Straight into Loop',
  'chat.adoptReadyHint': 'Queue it — with autopilot on, someone picks it up shortly',
  'chat.adopted': 'Created as {id}',
  'chat.adoptFailed': 'Could not create the card — try again',
  'chat.you': 'You',
  ...
  'editor.executor': 'Assigned to',
  'editor.executorHint': 'This card is theirs; @ someone else in the conversation to switch',
  'editor.executorDefault': 'Unset — the default executor “{name}” takes it',
  'editor.executorNone': 'No executors yet. Create one under Executors on the left.',
  'editor.executorAny': 'Default executor',
  'editor.executorFallback': 'Default executor ({name})',
  ...
  'err.executor-duplicate': 'An executor by that name already exists. Names have to be unambiguous.',
  'err.executor-empty': 'An executor needs a name.',
  'err.executor-too-long': 'That name is too long.',
  'err.executor-illegal-chars': 'Names can’t contain spaces, @ or #. That rule is what makes @ have an end.',
  'err.executor-unknown-provider': 'That CLI isn’t detected on this machine. Install it, or pick another.',
  'err.executor-not-found': 'That executor is gone — it may have just been deleted.',
  'err.chat-unavailable': 'This instance has no chat service, so there’s no one to talk to.',
  'err.proposal-not-found': 'That draft is gone.',
  'err.already-adopted': 'That draft is already a card.',
