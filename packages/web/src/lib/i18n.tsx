...
const zh = {
  ...
  'settings.title': '项目设置',
  'settings.hint': '项目的基本信息与测试环境都配在这里 —— 配一次，这个项目所有卡都照着跑。',
  ...
  'term.expand': '展开命令行',
  'term.collapse': '收起命令行',
  'term.placeholder': '跑一条命令',
  'term.stdinPlaceholder': '这一行发给正在跑的命令',
  'term.run': '执行',
  'term.send': '发给正在跑的命令',
  'term.clear': '清空',
  ...
  'testenv.filesLabel': '配置文件',
  'testenv.filesHint': '启动时从主工作区拷进这张卡的 worktree —— gitignore 掉的本地配置就靠它带过去。',
  'testenv.filesBrowse': '浏览文件',
  'testenv.filesNone': '还没选。点「浏览文件」，点到哪个文件哪个进清单。',
  'testenv.filesUp': '上一级',
  'testenv.filesRemove': '不再拷贝 {file}',
  ...
  // ── 决策：权限审批 / 向人提问 ───────────────────────────
  'decision.permissionTag': '权限请求',
  'decision.questionTag': 'Agent 想问你',
  'decision.timeoutHint': '10 分钟不处理会自动收场',
  'decision.allow': '允许',
  'decision.allowRun': '本次执行内总是允许',
  'decision.allowRunHint': '同一个工具在这次执行里不再询问 —— 只对这一次执行有效',
  'decision.deny': '拒绝',
  'decision.answer': '回答',
  'decision.answerPlaceholder': '回答会原样带给 Agent（⌘/Ctrl + Enter 发送）',
  'decision.failed': '这一下没送出去，稍后再试 —— Agent 还在那儿等着。',

  'term.empty': '还没跑过命令。',
  'term.hint': '这是个真会话：`cd` 留得住，长命令边跑边出，ctrl+c 打断。vim / top 这类要终端的程序在这儿跑不起来。',
  'term.cwd': '工作目录 {path}',
  'term.connecting': '正在开终端…',
  'term.running': '执行中…',
  'term.ok': '完成 · {ms}ms',
  'term.exit': '退出码 {code} · {ms}ms',
  'term.signal': '被 {signal} 终止 · {ms}ms',
  'term.interrupted': '已中断 · {ms}ms',
  'term.interrupt': '中断',
  'term.interruptHint': '^C 中断（整棵进程树）',
  'term.follow': 'cd 到浏览的目录',
  'term.followHint': 'cd 到上面正在浏览的目录 —— 终端有自己的目录，不会自动跟着走',
  'term.lost': '和这个终端的连接断了。它可能已经被回收（离开太久），正在跑的命令也一并收掉了。',
  'term.reconnect': '重新连接',
  ...
  'sidebar.newTaskPickProject': '新建到哪个项目？',
  // 现在概览里点新建会先弹项目清单，按钮灰着只剩一种情况：连一个项目都没有。
  'sidebar.newTaskBlocked': '还没有项目 —— 先建一个，任务得知道自己在哪个仓库里干活',
  'sidebar.overview': '概览',
  ...
  'card.blocked': '阻塞它的卡片数 —— 它们全部做完（进入 Done）这张卡才会被派出',
  'card.leaseExpired': '租约已过期 · 待回收',
  'card.pendingDecision': '停下来等你处理 · {n} 件',
  ...
  'panel.accept': '通过',
  'panel.discard': '废弃',
  'panel.acceptMergeArmed': '确认合并',
  'panel.acceptArmed': '确认通过',
  'panel.discardArmed': '确认废弃',
  'panel.archiveArmed': '确认归档',
  'panel.armedHint': '再点一次执行；点别的按钮会改判',
  ...
  // ── 右侧面板：建卡 + 一张卡的对话与配置 ─────────────────
  'chat.title': '聊天',
  'chat.hint': '写一句就是一张卡；选中一张卡，这儿就是它的对话',
  'chat.createHint': '写一句话，回车就建一张卡 —— 它落在想法池里，接着可以在「配置」里补验收标准、挑执行器。',
  'chat.createPlaceholder': '要做点什么？一句话就是一张卡',
  'chat.create': '建卡',
  'chat.createFailed': '这张卡没建成，稍后再试',
  'chat.saveFailed': '没存进去，稍后再试',
  'chat.moveFailed': '这张卡没挪动，稍后再试',
  'chat.noProject': '还没有项目。先在左侧新增一个 —— 卡总要长在某个仓库上。',
  'chat.project': '落到',
  'chat.created': '已建卡 {id}，在想法池里。',
  'chat.toReady': '移到 Ready',
  'chat.toReadyHint': '推进队列 —— 自动认领开着的话，很快就会有执行器把它捡走',
  'chat.tabTalk': '聊天',
  'chat.tabSpec': '配置',
  'chat.details': '详情',
  'chat.detailsHint': '打开详情：diff、事件流、PR、测试环境、权限审批都在那儿',
  'chat.deselect': '取消选中',
  'chat.deselectHint': '放开这张卡 —— 回到"写一句就是一张新卡"',
  'chat.collapse': '收起面板，看板占满',
  'chat.expand': '展开右侧的面板',

  // ── 一轮执行在对话里的样子 ──────────────────────────────
  'stream.live': '执行中',
  'stream.round': '第 {n} 轮',
  'stream.loading': '读取这一轮的过程…',
  'stream.truncated': '只摆了这一轮最后一段 —— 更早的在详情的事件流里。',
  'stream.ok': '完成',
  'stream.failed': '失败',
  'stream.waiting': '刚派出去，还没有动静…',
  'stream.empty': '这一轮没留下可读的过程 —— 原始输出在详情的事件流里。',
  'chat.runningLock': '正在执行 —— 等它跑完再说；它要是问你话，就在上面',
  ...
  'editor.modelNeedProvider': '先在左边挑一个执行器，才能列它的模型',
  'editor.permission': '权限档位',
  'editor.permissionHint': '决定 Agent 执行时的边界；不确定就用默认',
  'editor.permissionDefault': '（默认 · standard）',
  'editor.permissionUnsupported': '{provider} 不支持 {tier} 档，执行时会自动退回 standard 并在日志里说明',
  'editor.permission.tier.strict': 'strict · 未放行的一律拒绝',
  'editor.permission.tier.standard': 'standard · 交给 CLI 自己判断',
  'editor.permission.tier.supervised': 'supervised · 要权限时停下来问你',
  'editor.permission.tier.yolo': 'yolo · 全部放行，不看不要问',
  // ── 附件 ───────────────────────────────────────────────
  'editor.related': '关联任务',
  ...
  // ── 依赖 ───────────────────────────────────────────────
  'editor.blocked': '依赖任务',
  'editor.blockedHint': '这张卡要等它们全部做完（进入 Done）才会被派出去 —— 是硬约束，和「关联任务」的仅作参考不同。不能依赖自己，也不允许成环',
  'editor.blockedAdd': '＋ 依赖一张卡',
  'editor.blockedRemove': '取消这条依赖',
  'editor.blockedNone': '这个项目里还没有别的卡可以依赖',
  'editor.blockedAllPicked': '这个项目里的卡都已在依赖里',
  'editor.blockedGone': '这张卡已经不在了',
...
const en: Record<MessageKey, string> = {
  ...
  'settings.hint': 'Project basics and the test environment are configured here — once, and every card in this project runs by it.',
  ...
  'term.placeholder': 'Run a command',
  'term.stdinPlaceholder': 'This line goes to the running command',
  'term.run': 'Run',
  ...
  'testenv.filesLabel': 'Config files',
  'testenv.filesHint': 'Copied from the main worktree into this card’s worktree on start — that’s how gitignored local config gets there.',
  'testenv.filesBrowse': 'Browse files',
  'testenv.filesNone': 'Nothing picked yet. Hit “Browse files” and click your way to the ones you need.',
  'testenv.filesUp': 'Go up one level',
  'testenv.filesRemove': 'Stop copying {file}',
  ...
  'decision.permissionTag': 'Permission request',
  'decision.questionTag': 'The agent has a question',
  'decision.timeoutHint': 'Auto-resolved after 10 minutes',
  'decision.allow': 'Allow',
  'decision.allowRun': 'Always allow for this run',
  'decision.allowRunHint': 'This tool won’t be asked again during this run — it only lasts this run',
  'decision.deny': 'Deny',
  'decision.answer': 'Answer',
  'decision.answerPlaceholder': 'The answer goes to the agent as-is (⌘/Ctrl + Enter to send)',
  'decision.failed': 'That didn’t go through — try again; the agent is still waiting.',
  ...
  'term.cwd': 'in {path}',
  'term.connecting': 'opening the shell\u2026',
  'term.running': 'Running\u2026',
  'term.ok': 'done \u00b7 {ms}ms',
  'term.exit': 'exit {code} \u00b7 {ms}ms',
  'term.signal': 'killed by {signal} \u00b7 {ms}ms',
  'term.interrupted': 'interrupted \u00b7 {ms}ms',
  'term.interrupt': 'Interrupt',
  'term.interruptHint': '^C interrupts (whole process tree)',
  'term.follow': 'cd to the browsed directory',
  'term.followHint': 'cd to the directory shown above — the shell keeps its own, it does not follow along',
  'term.lost': 'Lost the connection to this shell. It may have been collected after sitting idle, taking any running command with it.',
  ...
  'sidebar.newTaskPickProject': 'New task in which project?',
  // The overview now pops a project list first, so the button is grey for one
  // reason only: there is no project to pick from at all.
  'sidebar.newTaskBlocked': 'No projects yet — add one first; a task has to know which repo it works in',
  ...
  'card.blocked': 'Blocking cards — dispatched only after all of them are done',
  'card.leaseExpired': 'Lease expired · to be reclaimed',
  'card.pendingDecision': 'Waiting for you · {n}',
  ...
  'panel.acceptMergeArmed': 'Confirm merge',
  'panel.acceptArmed': 'Confirm accept',
  'panel.discardArmed': 'Confirm discard',
  'panel.archiveArmed': 'Confirm archive',
  'panel.armedHint': 'Click again to go through; clicking another button switches instead',
  ...
  'chat.title': 'Chat',
  'chat.hint': 'A line here is a new card; pick a card and this becomes its conversation',
  'chat.createHint': 'Write a line and hit Enter to create a card. It lands in the idea pool — fill in acceptance criteria and pick an agent under Config.',
  'chat.createPlaceholder': 'What needs doing? One line is one card',
  'chat.create': 'Create',
  'chat.createFailed': 'Could not create the card — try again',
  'chat.saveFailed': 'Could not save — try again',
  'chat.moveFailed': 'Could not move the card — try again',
  'chat.noProject': 'No projects yet. Add one on the left — a card has to live in some repo.',
  'chat.project': 'Into',
  'chat.created': 'Created {id}, sitting in the idea pool.',
  'chat.toReady': 'Move to Ready',
  'chat.toReadyHint': 'Put it in the queue — with autopilot on, an agent will pick it up shortly',
  'chat.tabTalk': 'Chat',
  'chat.tabSpec': 'Config',
  'chat.details': 'Details',
  'chat.detailsHint': 'Open details: diff, event stream, PR, test environment and approvals live there',
  'chat.deselect': 'Deselect',
  'chat.deselectHint': 'Let go of this card — back to "a line here is a new card"',
  'chat.collapse': 'Collapse the panel and give the board the full width',
  'chat.expand': 'Open the panel on the right',

  'stream.live': 'running',
  'stream.round': 'Round {n}',
  'stream.loading': 'Loading this round…',
  'stream.truncated': 'Only the tail of this round is shown — the rest is in the event stream under Details.',
  'stream.ok': 'done',
  'stream.failed': 'failed',
  'stream.waiting': 'Just dispatched — nothing yet…',
  'stream.empty': 'This round left nothing readable — the raw output is in the event stream under Details.',
  'chat.runningLock': 'Running — wait for this round; if it asks you something, it is right above',
  ...
  'editor.modelNeedProvider': 'Pick an agent on the left to list its models',
  'editor.permission': 'Permission tier',
  'editor.permissionHint': 'Draws the line the agent works within; when unsure, keep the default',
  'editor.permissionDefault': '(default · standard)',
  'editor.permissionUnsupported': '{provider} doesn’t support the {tier} tier — the run falls back to standard and says so in its log',
  'editor.permission.tier.strict': 'strict · deny everything not pre-approved',
  'editor.permission.tier.standard': 'standard · let the CLI decide',
  'editor.permission.tier.supervised': 'supervised · pause and ask you for permissions',
  'editor.permission.tier.yolo': 'yolo · allow everything, ask nothing',
  ...
  'editor.blocked': 'Dependencies',
  'editor.blockedHint': "This card won't be dispatched until all of them are done — a hard constraint, unlike the read-only references above. No self-dependency, and cycles are rejected",
  'editor.blockedAdd': '+ Depend on a card',
  'editor.blockedRemove': 'Remove this dependency',
  'editor.blockedNone': 'No other card in this project to depend on yet',
  'editor.blockedAllPicked': 'Every card in this project is already a dependency',
  'editor.blockedGone': 'This card is gone',
